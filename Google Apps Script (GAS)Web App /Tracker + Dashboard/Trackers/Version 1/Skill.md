...
- **Single File Output**: Provide the frontend code as a single `index.html` file.
- **Embedded Assets**: Place all CSS strictly inside `<style>` tags and all JavaScript inside `<script>` tags within the `index.html` file. GAS `HtmlService` does not natively support external `.css` or `.js` file linking from the workspace context.
...
- **Entry Point**: Always include the `doGet(e)` function to serve the web app interface using `HtmlService.createHtmlOutputFromFile('index')`.
